...
<a name='L255'></a><a href='#L255'>255</a></td><td class="line-coverage quiet"><span class="cline-any cline-yes">12x</span>
...
export interface GeocoderProps {
  ...
  /**
   * Fired when the user is typing in the input and provides a list of features.
   * The underlying response from {@link MapboxGeocode} is passed.
   */
  onResults?: (results: GeocodeResponse) =&gt; void;
  /**
   * Fired when {@link MapboxGeocode} has errored providing a list of features.
   * The underlying error is passed.
   */
  onError?: (error: Error) =&gt; void;
  /**
   * Fired when the user has selected a feature.
   * The underlying response from {@link MapboxGeocode} is passed.
   */
...
&nbsp;
/**
 * `&lt;Geocoder&gt;` is a React component that provides an interactive geocoding box,
 ...
 *
 * Internally, this wraps the [`&lt;mapbox-geocoder&gt;`](https://docs.mapbox.com/mapbox-search-js/api/web/geocode/#mapboxgeocoder) element.
 *
 * @function Geocoder
...
&nbsp;
      if (!onResults) return;
&nbsp;
      const fn = (e: MapboxHTMLEvent&lt;GeocodeResponse&gt;) =&gt; onResults(e.detail);
&nbsp;
      node.addEventListener('results', fn);
      return () =&gt; {
        node.removeEventListener('results', fn);
      };
    }, [ref.current, onResults]);
&nbsp;
    // Update onError.
    useEffect(() =&gt; {
      const node = ref.current;
...
&nbsp;
      if (!onError) return;
&nbsp;
      const fn = (e: MapboxHTMLEvent&lt;Error&gt;) =&gt; onError(e.detail);
&nbsp;
      node.addEventListener('error', fn);
      return () =&gt; {
        node.removeEventListener('error', fn);
      };
    }, [ref.current, onError]);
&nbsp;
    // Update onSelect.
...
&nbsp;
      if (!onSelect) return;
&nbsp;
      const fn = (e: MapboxHTMLEvent&lt;GeocodeFeature&gt;) =&gt; onSelect(e.detail);
&nbsp;
      node.addEventListener('select', fn);
      return () =&gt; {
        node.removeEventListener('select', fn);
      };
...
&nbsp;
      const fn = (e: MapboxHTMLEvent&lt;string&gt;) =&gt; {
        <span class="missing-if-branch" title="if path not taken" >I</span>if (e.target !== e.currentTarget) <span class="cstat-no" title="statement not covered" >return; // ignore child input event</span>
        onChange(e.detail);
      };
&nbsp;
...
&nbsp;</pre></td></tr></table></pre>
            ...
            <div class='footer quiet pad2 space-top1 center small'>
                ...
                at Thu Jul 07 2022 10:18:27 GMT-0700 (Pacific Daylight Time)
            </div>
